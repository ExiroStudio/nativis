...
use nativis_rhi::{IRhiBackend, TextureDescriptor, TextureFormat, TextureUpload};
...
impl IMediaSource for ImageSource {
    ...
    fn initialize(&mut self, rhi: &mut dyn IRhiBackend) -> Result<(), MediaError> {
        ...
        let desc = TextureDescriptor::sampled_2d(self.width, self.height, TextureFormat::Rgba8Unorm);
        let handle = rhi.create_texture(&desc)
            .map_err(|e| MediaError::GpuUpload(e.to_string()))?;

        rhi.upload_texture_data(TextureUpload {
            handle,
            data: img.as_raw(),
